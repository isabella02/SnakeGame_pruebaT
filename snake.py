...
class Player(object):
    ...
    def hacer_movimiento(self, x, y, game, food, damaged_food):
        array_m = [self.cambio_x, self.cambio_y]

        imagen_pausa = pygame.image.load('img/pausa.png')
        imagen_pausa = pygame.transform.scale(imagen_pausa, (100, 100))
        ...
        for e in pygame.event.get():
            if e.type == pygame.KEYDOWN:
                if e.key == pygame.K_LEFT and self.direccion != [1, 0]:
                    ...
                    pausado = True
                    while pausado:
                        game.display_juego.blit(imagen_pausa, (0, 0))
                        ...
                        for evento in pygame.event.get():
                            if evento.type == pygame.KEYDOWN:
                                ...
                                # Salir del juego y regresar al menú cuando se presiona la tecla 'M'
                                elif evento.key == pygame.K_m:
                                    velocidad_serpiente = mostrar_menu()
                                    run(velocidad_serpiente)
        ...
        if (
            self.x < 0
            or self.x > game.ancho_juego - 20
            or self.y < 0
            or self.y > game.alto_juego - 20
            or [self.x, self.y] in self.posicion
        ):
            game.colision = True

        food.comer(self, game)
        damaged_food.comer(self, game)
...
class DamagedFood(Food):
    def __init__(self):
        self.x_food = 500
        self.y_food = 200
        self.imagen = pygame.image.load('img/comidaDañada.png') 

    def comer(self, player, game):
        if player.x == self.x_food and player.y == self.y_food:
            self.comida_coor(game, player)
            player.comidaDañada = True
            game.score = 0
            #game.colision = True
...
def mostrar_menu():
    ...
    pygame.display.set_caption('Snake - Menú de Dificultad')

    fondo = pygame.image.load('img/menu.png')
    fondo = pygame.transform.scale(fondo, (400, 300))

    fuente_grande = pygame.font.SysFont('Ubuntu', 30)
    fuente_pequena = pygame.font.SysFont('Ubuntu', 20)

    facil = fuente_pequena.render('1. Fácil', True, (0, 128, 0))
    normal = fuente_pequena.render('2. Normal', True, (0, 128, 0))
    dificil = fuente_pequena.render('3. Difícil', True, (0, 128, 0))

    pantalla.blit(fondo, (0, 0))
    pantalla.blit(facil, (160, 140))
    pantalla.blit(normal, (160, 170))
    pantalla.blit(dificil, (160, 200))
        ...
        for evento in pygame.event.get():
            if evento.type == pygame.QUIT:
                ...
                if evento.key == pygame.K_1:
                    return 5  # Dificultad Fácil
                elif evento.key == pygame.K_2:
                    return 10  # Dificultad Normal
                elif evento.key == pygame.K_3:
                    return 15  # Dificultad Difícil

            elif evento.type == pygame.MOUSEBUTTONDOWN:
                x, y = evento.pos
                if 160 < x < 300 and 140 < y < 170:
                    return 5  # Dificultad Fácil
                elif 160 < x < 300 and 170 < y < 200:
                    return 10  # Dificultad Normal
                elif 160 < x < 300 and 200 < y < 230:
                    return 15  # Dificultad Difícil

def run(velocidad_serpiente):
    pygame.init()
    record = 0
    clock = pygame.time.Clock()
    last_damaged_food_time = time.time()

    while True:
        game = Game(420, 420)
        player = Player()
        food = Food()
        damaged_food = DamagedFood()
        ...
        while not game.colision:
            player.hacer_movimiento(player.x, player.y, game, food, damaged_food)
            record = game.obtener_record(game.score, record)
            game.display_ui(record)
            player.display_jugador(player.x, player.y, game)

            # Verifica si es hora de mostrar la comida dañada y lo hace cada 10 segundos
            current_time = time.time()
            if current_time - last_damaged_food_time > 10:
                damaged_food.comida_coor(game, player)
                last_damaged_food_time = current_time
            damaged_food.display_comida(damaged_food.x_food, damaged_food.y_food, game)

            food.display_comida(food.x_food, food.y_food, game)

            pygame.display.update()
            clock.tick(velocidad_serpiente)
